 ...
 *
 *BitSet类实现了一个按需增长的位向量。位 set 的每个组件都有一个boolean值。用非负的整数将BitSet的位编入索引。可以对每个编入索引的位进行测试、设置或者清除。通过逻辑与、逻辑或和逻辑异或操作，可以使用一个BitSet修改另一个BitSet的内容。
...
public class BitSetExample {
    ...
    public static void main(String args[]) {
        //BitSet使用示例
        BitSetExample.containChars("How do you do? 你好呀");
        BitSetExample.computePrime();
        BitSetExample.sortArray();
        BitSetExample.simpleExample();
        ...
        //将BitSet对象转成byte数组
        byte[] bytes = BitSetExample.bitSet2ByteArray(bitSet);
        System.out.println(Arrays.toString(bytes));

        //在将byte数组转回来
        bitSet = BitSetExample.byteArray2BitSet(bytes);
        System.out.println(bitSet.size()+","+bitSet.cardinality());
